...
namespace KollectorScum.Api.Repositories
{
    ...
    public class UnitOfWork : IUnitOfWork
    {
        ...
        /// <summary>
        /// Atomically upserts a Packaging row and returns its Id.
        /// </summary>
        public async Task<int> UpsertPackagingAsync(Guid userId, string name)
        {
            return await _context.UpsertPackagingAsync(userId, name);
        }

        /// <summary>
        /// Resets all lookup-table PostgreSQL sequences to the current maximum Id so that
        /// new inserts do not collide with rows that were seeded with explicit Ids.
        /// </summary>
