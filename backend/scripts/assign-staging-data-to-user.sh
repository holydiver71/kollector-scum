...
usage() {
  cat >&2 <<'EOF'
Assign staging data to a specific user.

Why this exists:
- Multi-tenant support scopes data by UserId.
- Imported/copied data may be owned by the seeded admin user.

This script updates all tables with a UUID "UserId" column, changing ownership from the
seeded admin user to the chosen user's Id.

Usage:
  backend/scripts/assign-staging-data-to-user.sh --email <email> --diagnose
  backend/scripts/assign-staging-data-to-user.sh --email <email> --yes [--from-user-id <uuid>]

Notes:
  - Requires staging DB URL in repo-root .env: KOLLECTOR_STAGING_DB_URL
  - The target user must already exist in staging (sign in once via Google first).
  - This only touches staging.
EOF
}
...
# Lookup the target user's Id by email.
TARGET_USER_ID="$(
  psql "$STAGING_URL_SSL" -v ON_ERROR_STOP=1 -v email="$EMAIL" -qAt <<'SQL'
SELECT "Id"::text
...
if [[ -z "$TARGET_USER_ID" ]]; then
  echo "No ApplicationUsers row found for email: $EMAIL" >&2
...
if [[ "$DIAGNOSE_ONLY" == "true" ]]; then
  echo "Diagnosing staging ownership for user: $EMAIL ($TARGET_USER_ID)"
  echo "Default source owner (seeded admin): $ADMIN_USER_ID"
...
SQL
  ...
  echo "Diagnosis complete. If data exists but is owned by a different user, re-run with:"
  echo "  backend/scripts/assign-staging-data-to-user.sh --email $EMAIL --yes --from-user-id <owner-uuid>"
  exit 0
fi

echo "Reassigning staging data from ($FROM_USER_ID) -> user ($TARGET_USER_ID)"

# Update every table that has a UUID UserId column.
# IMPORTANT: bash expands $$ to PID. Use \$\$ so psql receives literal $$.
psql "$STAGING_URL_SSL" -v ON_ERROR_STOP=1 -q <<SQL
DO \$\$
DECLARE
  r RECORD;
...
echo "Done. Refresh the app; you should now see the imported staging data under $EMAIL."
