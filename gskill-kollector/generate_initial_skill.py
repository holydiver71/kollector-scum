...
GITHUB_SKILL_PATH = Path(".github/skills/kollector-scum/SKILL.md")
...
def render_github_skill(body: str) -> str:
    """Render the shared markdown body as a SKILL.md for GitHub copilot skill directory."""
    ...
    frontmatter = textwrap.dedent(
        ...
        name: kollector-scum
        version: "0.1.0"
        description: "Copilot skill for contributing to the kollector-scum music catalog SaaS"
...
def write_outputs(repo_root: Path, body: str) -> tuple[Path, Path]:
    """Write skill files to their repository locations: Claude and GitHub skill dirs."""

    claude_path = repo_root / CLAUDE_SKILL_PATH
    github_skill_path = repo_root / GITHUB_SKILL_PATH

    claude_path.parent.mkdir(parents=True, exist_ok=True)
    github_skill_path.parent.mkdir(parents=True, exist_ok=True)

    claude_path.write_text(render_claude_skill(body), encoding="utf-8")
    github_skill_path.write_text(render_github_skill(body), encoding="utf-8")

    return claude_path, github_skill_path
