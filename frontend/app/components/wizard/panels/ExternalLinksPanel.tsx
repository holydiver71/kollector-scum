...
import type { WizardFormData, WizardLink, ValidationErrors } from "../types";

/** Static list of recognised external link service types */
const LINK_TYPES = [
  "Discogs",
  "Spotify",
  "YouTube",
...
interface Props {
  ...
  /** Per-field validation errors (keyed as "link{index}") */
  errors: ValidationErrors;
}

/**
 * Panel 7 – External Links (optional).
 * Allows multiple URL + type + description triples.
 * New links default to type "Discogs".
 */
export default function ExternalLinksPanel({ data, onChange, errors }: Props) {
  const links: WizardLink[] = data.links ?? [];

  const updateLinks = (updated: WizardLink[]) => onChange({ links: updated });
  ...
  const updateLink = (index: number, patch: Partial<WizardLink>) => {
        ...
        {links.map((link, index) => (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label
                  htmlFor={`wiz-link-type-${index}`}
                  className="block text-xs font-semibold text-[#A78BFA]/60 uppercase tracking-wider mb-1.5"
                >
                  Type
                </label>
                <select
                  id={`wiz-link-type-${index}`}
                  value={link.type}
