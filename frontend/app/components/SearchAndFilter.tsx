...
export function SearchAndFilter({ onFiltersChange, initialFilters, enableUrlSync = false, showSearchInput = true, openAdvanced, onAdvancedToggle, kollectionId, compact = false }: SearchAndFilterProps) {
  ...
  return (
    <div 
      className={`relative bg-[#13131F] rounded-2xl border border-[#1C1C28] ${compact ? 'px-4 py-3 mb-0' : 'p-6 mb-4'} text-white overflow-visible`}
    >
      <div className={`relative z-10 ${compact ? 'space-y-2' : 'space-y-4'}`}>
        {/* Search Input with Autocomplete */}
        {showSearchInput && (
        <div>
          <label htmlFor="search" className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-2.5 bg-[#0A0A10] border border-[#2E2E3E] rounded-xl focus:outline-none focus:border-[#8B5CF6] text-white placeholder-gray-600 transition-all"
              autoComplete="off"
            />
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div 
                ref={suggestionsRef}
                className="absolute z-10 w-full mt-1 bg-[#13131F] border border-[#1C1C28] rounded-xl shadow-2xl max-h-60 overflow-auto"
              >
                {suggestions.map((suggestion, index) => (
                  <button
                    key={`${suggestion.type}-${suggestion.id}`}
                    onClick={() => handleSuggestionClick(suggestion)}
                    className={`w-full text-left px-4 py-3 hover:bg-[#1C1C28] flex items-center justify-between transition-colors border-b border-[#1C1C28]/50 last:border-0 ${
                      index === suggestionIndex ? 'bg-[#8B5CF6]/10' : ''
                    }`}
                  >
                    <div>
                    ...
                    <span className="text-xs text-[#A78BFA] capitalize px-2 py-1 bg-[#8B5CF6]/10 rounded-full">{suggestion.type}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Recording Type</label>
              <LookupDropdown
            ...
            {/* Year Range Filters */}
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">From Year</label>
              <input
                type="number"
                ...
                className="w-full px-3 py-2.5 bg-[#0A0A10] border border-[#2E2E3E] rounded-xl focus:outline-none focus:border-[#8B5CF6] text-white placeholder-gray-600 transition-all text-sm"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">To Year</label>
              <input
                type="number"
  ...
  return (
    <form ref={containerRef} onSubmit={handleSubmit} className="relative">
      ...
      <input
        ...
        onFocus={() => suggestions.length > 0 && setShowSuggestions(true)}
        className="block w-full pl-10 pr-10 py-2 border border-[#2E2E3E] rounded-xl focus:outline-none focus:border-[#8B5CF6] bg-[#0A0A10] focus:bg-[#0A0A10] text-white placeholder-gray-600 transition-all"
        autoComplete="off"
      />
      {query && (
        <button
      ...
      {showSuggestions && suggestions.length > 0 && dropdownStyle && createPortal(
        <div ref={dropdownRef} style={dropdownStyle} className="bg-[#13131F] border border-[#1C1C28] rounded-xl shadow-2xl max-h-60 overflow-auto">
          {suggestions.map((s, idx) => (
            <button
              key={`${s.type}-${s.id}`}
              type="button"
              onClick={() => handleSuggestionClick(s)}
              className={`w-full text-left px-4 py-3 hover:bg-[#1C1C28] flex items-center justify-between transition-colors border-b border-[#1C1C28]/50 last:border-0 ${idx === suggestionIndex ? 'bg-[#8B5CF6]/10' : ''}`}
            >
              <div>
                <div className="font-medium text-white">{s.name}</div>
                {s.subtitle && <div className="text-sm text-gray-400">{s.subtitle}</div>}
              </div>
              <span className="text-xs text-[#A78BFA] capitalize px-2 py-1 bg-[#8B5CF6]/10 rounded-full">{s.type}</span>
            </button>
          ))}
        </div>,
        document.body
